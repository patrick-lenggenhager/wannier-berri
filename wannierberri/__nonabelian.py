...
TRodd  = set(['spin','morb','morbg','vel','curv','curvE'])
INVodd = set(['vel'])
...
def Morb(data,Efermi,degen_thresh):
    ...
    r3.data[:,:]=-2*r3.data[:,:]*Efermi[:,None]
    return (r1+(r2+r3))*__berry.fac_morb*data.cell_volume


def Morbg(data,Efermi,degen_thresh):
    return nonabelian_general(data,Efermi,['morbg'],degen_thresh=degen_thresh,mode='fermi-sea',
              factor=__berry.fac_morb*data.cell_volume)



## this is equavalent to the previous routine - checked
